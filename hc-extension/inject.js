(async function() {
  ...
  // chrome.storageからデッキデータを取得（MAIN worldでも使えるか確認）
  // → MAIN worldではchrome APIが使えないのでlocalStorageを使う
  const raw = localStorage.getItem('hc_deck_data');
  if (!raw) return; // データがなければ何もしない
  localStorage.removeItem('hc_deck_data'); // 使い捨て
  ...
  const selType = async label => {
    const inp = Array.from(document.querySelectorAll('input[type=radio]'))
      .find(r => {
